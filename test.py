...
"""
@Authors: Adarsh Malapaka, Kumara Ritvik Oruganti, Sandeep Adapa
"""
...
server_socket.bind(('172.20.10.3', 7999))  # ADD IP HERE
...
print("Path: ", PATH)
assert len(PATH) == 1
model = PathModel.load_from_checkpoint(PATH[0], arch="Unet", encoder_name="mobilenet_v2", in_channels=3, out_classes=1, mode=mode, strict=False)
model.cuda()
#output path
...
try:
    ...
    while True:
        ...
        pr_mask = pr_mask*128
        # print(np.min(pr_mask))
        green_mask[:,:,1][pr_mask<64] = 0

        # Segmented mask overlayed on original image
        ...
        # Warping the segmented image to a different perspective
        # (0,720) (400,170) (840, 170) (1280, 720) to (0,720) (0, 0) (1280, 0), (1280, 720)
        # Specify input and output coordinates that is used to calculate the Homogeneous transformation matrix
        
        # Apply the perspective transformation to the image
        n_mask = cv2.resize(np.uint8(pr_mask).copy(), (1280,720), cv2.INTER_LINEAR)
        out = cv2.warpPerspective(n_mask, M, (image.shape[1], image.shape[0]), flags=cv2.INTER_LINEAR)

        # cv2.imshow("N IMAGE", out)
        out = cv2.resize(out,(360,900),interpolation=cv2.INTER_LINEAR) #Resize the image
